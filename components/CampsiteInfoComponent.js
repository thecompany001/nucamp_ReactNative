...
function RenderCampsite(props) {
	...
	const panResponder = PanResponder.create({
		...
		onPanResponderGrant: () => {
			view.current.rubberBank(1000)
			.then(endState => console.log(endState.finished ? 'finished' : 'canceled'));
		},
		onPanResponderEnd: (e, gesturesState) => {
			console.log('pan responder end', gesturesState);
			if (recognizeDrag(gesturesState)) {
				Alert.alert(
					'Add Favorite',
			...
			<Animatable.View 
				...
				<Card featuredTitle={campsite.name} image={{ uri: baseUrl + campsite.image }}>
					...
					<View style={styles.cardRow}>
						...
						<Icon
							name={'share'}
							type='font-awesome' color='#5637DD' raised reverse onPress={() => shareCampsite(campsite.name, campsite.description,
...
function RenderComments({ comments }) {
	...
	return (
		<Animatable.View animation='fadeInUp' duration={20000} delay={1000}>
			<Card title='Comments'>
